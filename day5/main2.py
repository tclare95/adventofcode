...
def process_instruction(str, data_array):
    ...
    # select the number of items from the source position as defined by the num_items
    items = data_array[source][1][-num_items:]
    # appened the items to the destination position
    data_array[dest][1].extend(items)
    # remove the items from the source position
    del data_array[source][1][-num_items:]
    return data_array
